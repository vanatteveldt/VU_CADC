...
author: "prof. dr. Wouter van Atteveldt & dr. Philipp K. Masur"
format:
  revealjs: 
...
#| class-output: output
library(tokenizers)
text_data <- tibble(doc_id = paste0("text", 1:2),
...
| subword tokens | "I", "love", "d", "you", "enorm", "ous", "ly" |
...
#| class-output: output
tokens |>
  group_by(doc_id, word) |>          # <-- group by document id and word
...
sotu_text <- bind_cols(sotu_meta, text = sotu_text) |>  # <-- bind texts and meta data
  rename(doc_id = X) |>                                  # <-- rename unique identifier
...
tidy_text <- sotu_text |>
  ...
  group_by(doc_id, word) |>                             # <-- group by document id and words
  summarise(n = n()) |>                                # <-- summarize number of words per document
...
#dfm_text <- tidy_text |> 
#  cast_dfm(document = doc_id, term = word,  value = n)  # <-- create document-term matrix
#dfm_text
```
...
-   Representing all the numbers including zeroes is a 'dense' representation,
    which is very memory inefficient
...
frequencies <- sotu_tokens |>          
  ...
  summarize(termfreq=n(),                               # <-- computer overall occurrences of a word
            docfreq=length(unique(doc_id)),              # <-- number of speeches in which it occurs
...
tidy_kwic(sotu_text, "terror", window = 5) |>
  select(pre:doc_id, president, year) |>
  head(n = 18)
```
...
ggplot() +
 ...
 geom_line(data = sotu_plot_data |> filter(doc_id == 1), 
